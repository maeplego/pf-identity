...
- SAML、CIBA、PAR
- Back-Channel Logout（他 RP へのサーバー間通知）
- ソーシャルログイン
...
RP-Initiated Logout は Discovery の `end_session_endpoint`（`/end-session`）。`post_logout_redirect_uri` は登録値と完全一致。`id_token_hint` が現在のセッションと一致すれば確認画面を出さない。同じ OP セッションでログインした RP には Front-Channel Logout（隠し iframe、`iss` と `sid`）を送る。
